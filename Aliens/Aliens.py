'''This is the original alien dropping game.
Made before I knew how to return anything :(''' 
...
def create_object(obj_data, wipeout_data, buff_data):
    if len(obj_data) < 10 and random.random() < 0.05:
        x = random.randint(0, win_width - obj_size)
        y = 0
        obj_data.append([x, y, obj])

    if len(wipeout_data) < 6 and random.random() < 0.03:
        x = random.randint(0, win_width - obj_size)
        y = 0
        wipeout_data.append([x, y, wipeout])
    
    if len(buff_data) < 5 and random.random() < 0.01:
        x = random.randint(0, win_width - buff_size)
        y = 0
        buff_data.append([x, y, buff])
...
def update_buffs_special(buff_data):
    global health

    for object in buff_data:
        x, y, image_data = object
        if y < win_height:
            y += speed * 0.7
            object[1] = y
...
def collision_check(obj_data, wipeout_data, buff_data, player_pos):
    ...
    for object in buff_data:
        x, y, image_data = object
        player_x, player_y = player_pos[0], player_pos[1]
        buff_rect = pg.Rect(x, y, buff_size, buff_size)
        player_rect = pg.Rect(player_x, player_y, player_size, player_size)
            
        if player_rect.colliderect(buff_rect):
            health += 5
            heal = pg.mixer.Sound('heal.mp3')
            heal.play()
            buff_data.remove(object)

    for object in wipeout_data:
        x, y, image_data = object
        player_x, player_y = player_pos[0], player_pos[1]
        wipeout_rect = pg.Rect(x, y, wipeout_size, wipeout_size)
        player_rect = pg.Rect(player_x, player_y, player_size, player_size)
        
        if player_rect.colliderect(wipeout_rect):
            die()
            time.sleep(2)
            running = False
            break
...
while running:
    ...
    create_object(obj_data, wipeout_data, buff_data)

    update_objects(obj_data)
    update_objects(wipeout_data)
    update_buffs_special(buff_data)

    collision_check(obj_data, wipeout_data, buff_data, player_pos)
